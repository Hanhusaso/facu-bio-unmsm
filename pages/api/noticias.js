	...
	try {
		...
		if (categorias.length != 0) {
			for (var i = 0; i < categorias.length; i++) {
				query = query + `&_where[_or][${i}][sub_categoria_noticia.nombre]=${categorias[i]}`;
			}
		}
		...
		const url = `${BASE_PATH}/novedades?_sort=fecha:DESC&${limitItems}&${startItems}${query}`;
		const response = await fetch(url);
		const result = await response.json();
		return result;
	} catch (error) {
...
export async function countNoticiasApi(palabra, categorias, date) {
	try {
		...
		const url = `${BASE_PATH}/novedades/count${query}`;
		const response = await fetch(url);
		const result = await response.json();
		return result;
	} catch (error) {
